...
import { PlanStore, PlanStoreLayer } from "./services/plan-store.ts";
import { HarnessToolkit, HarnessToolkitLayerBare } from "./toolkit.ts";
...
const modelOption = Options.text("model").pipe(
  Options.withDefault("claude-sonnet-4-6"),
...
const harness = Command.make("harness").pipe(Command.withSubcommands([runCommand]));
