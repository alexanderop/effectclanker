...
- Docs snapshot path: `/tmp/docs-snapshot.md` — Read this one file for all doc content
- Vault path: `docs/`
- Use Glob/Grep only for **codebase verification**, not for reading doc/skill files
- Return a structured markdown report

## Auditor

**Inputs:** docs snapshot, auto-memory directory path, CLAUDE.md path.

Prompt spec:

- Read `/tmp/docs-snapshot.md` and parse it to build a wikilink map — no individual file reads needed
...
- Flag notes as:
  ...
  - **Orphaned**: Exists on disk but is not linked from any index (`docs/index.md`, `docs/principles.md`, `docs/plans/index.md`) or other doc file
- **Audit CLAUDE.md**: Flag sections that are outdated, redundant with doc notes or skill instructions, or could be condensed. Check that instructions match actual project structure.
- **Audit auto-memory files**: Read MEMORY.md and linked files. Flag:
  ...
  - **Duplicated in docs**: Entries that duplicate doc note content
- Produce a report with docs, CLAUDE.md, and memory findings separated. Each item: what's flagged, why, and suggested action (update, merge, condense, or delete).

## Reviewer

**Inputs:** docs snapshot, skills snapshot (`/tmp/skills-snapshot.md`), auditor report, `docs/principles.md`.
...
- Focus on codebase notes, preferences, and gotchas under `docs/patterns/`, `docs/guides/`, and `docs/stories/`
...
- Each proposed principle: insight, evidence (which notes), why independent, suggested path under `docs/principles/`

**Section 3 — Skill review:**
- For each skill, check against docs principles:
...
- X entries flagged (Y outdated, Z redundant, V stale session state, U duplicated in docs)
- [list each with one-line reason]
